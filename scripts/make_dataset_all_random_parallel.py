...
def get_args():
    ...
    parser.add_argument("--binary_path", type=str, default="./gen_auto")
...
def build_payload_random(batch_size: int) -> bytes:
    return ("1\n" * batch_size).encode()


def run_c_batch(binary_path: str, payload: bytes, n: int, m: int, t: int, alpha_m: int):
    env = os.environ.copy()
    # utile si gen_auto ou ses libs utilisent OpenMP / BLAS
    env["OMP_NUM_THREADS"] = "1"
    env["OPENBLAS_NUM_THREADS"] = "1"
    env["MKL_NUM_THREADS"] = "1"

    res = subprocess.run(
        [binary_path, str(m), str(t), str(n), str(alpha_m)],
        input=payload,
        capture_output=True,
        check=True,
        env=env,
...
def worker(worker_id, samples_to_generate, n, m, t, batch_size, binary_path, out_dir):
    # important: éviter que tous les process héritent exactement du même état aléatoire
    ...
    while written < samples_to_generate:
        ...
        payload = build_payload_random(current_batch)
        stdout_data, stderr_data = run_c_batch(binary_path, payload, n, m, t, 0)
...
def main():
    ...
    batch_size = 1000         
    num_workers = 10
    binary_path = "./generationC/gen_random"

    out_dir = f"../ai4code/data/dataset_random_{n}_H5/AT_random_nmt_{n}_{m}_{t}"
